...
In the one shot game the Nash equilibrium\footnote{\red{explain Nash eq. in this footnote and give ref.}} is for all players to chose defect. However in an iterated version there might be a gain for both players to choose some other strategy. The strategy \textco{all-cooperate} for example would give a better dividends if it is cheered by both players, but \textco{tit-for-tat} is even better. % explanation!
\mypar

% the noise
To make \red{\dots} \textco{nose} is introduced as a probability for players to make a mistake and choose the action that was not intended. The probability $p_{\mathrm{err}}$ corresponds to the chance in one round for one player to make an error. % This needs to be explained further.
\mypar
